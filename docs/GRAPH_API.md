...
| `FindByKind` | Filters by the stringified `Symbol` and pages results (`kernel/src/graph/store.rs::find_by_kind`). | Currently returns **all** Things regardless of `kind` (development shortcut). | Both implementations share the same wire format. |
