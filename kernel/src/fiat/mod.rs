...
pub fn parse_and_materialize_bootstrap(
    ...
    use crate::{SymbolTable, parse_fiat_line};
    let mut symbol_table = SymbolTable::default();
    let facts: Vec<_> = BOOTSTRAP_PAGING_DSL
        .lines()
...
fn parse_page_address(name: &str) -> Option<VirtAddr> {
    if name.starts_with("heap_page_0") {
        Some(VirtAddr::new(crate::HEAP_START))
    } else if name.starts_with("heap_page_1") {
        Some(VirtAddr::new(crate::HEAP_START + 0x1000))
    } else {
        None
    }
}
...
impl SymbolTable {
    pub fn resolve_or_create(&mut self, name: &str) -> String {
        if let Some(uuid) = self.names.get(name) {
            ...
            self.names.insert(name.to_string(), uuid.clone());
            self.reverse.insert(uuid.clone(), name.to_string());
            uuid
        }
    }
