...
/// Initialize all interrupts: disable PIC, enable APIC, and set up LAPIC timer
pub fn init_interrupts() {
    disable_pic();
    enable_apic();
    setup_apic_timer();

    // Switch to the dedicated kernel stack (set RSP to the new stack top)
    let stack_top = crate::stack::STACK_TOP;
    unsafe {
        asm!(
            "mov rsp, {0}",              // Load new stack pointer
            in(reg) stack_top,
            options(nostack)
        );
    }
    log::info!("1 Kernel stack switched to {:#x}", stack_top);

    // Now enable interrupts, since a valid stack is in place
    log::info!("Enabling interrupts.");
    x86_64::instructions::interrupts::enable(); // Equivalent to `sti`
