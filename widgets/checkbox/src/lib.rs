...
impl WidgetAbi for CheckboxWidget {
    type State = State;

    fn init(ctx: &WidgetContext) -> Self::State {
        let mut label = String::from("Checkbox");
        let mut checked = false;

        let req = GraphPropsGetRequest {
            node: ctx.widget_id,
            keys: alloc::vec![canon::TEXT, CHECKED],
        };
        ...
        if let Some(props) = get_props(req) {
            ...
            if let Some(Value::Bool(b)) = props.get(&CHECKED) {
                checked = *b;
            }
        }
    ...
    fn draw(state: &Self::State, fb: &mut [u8], rect: Rect) {
        ...
        for y in 0..box_size {
            for x in 0..box_size {
                let px = box_x + x;
                let py = box_y + y;
                if px >= 0 && px < rect.width as i32 && py >= 0 && py < rect.height as i32 {
        ...
        if state.checked {
            let check_color: u32 = 0xFF_00_00_00;
            // Simple X or check
            let cx = box_x + 3;
            let cy = box_y + 3;
            for i in 0..6 {
                ...
                let py = cy + i;
                let offset = ((py as usize * rect.width as usize) + px as usize) * 4;
                fb[offset] = (check_color >> 16) as u8;
                fb[offset + 1] = (check_color >> 8) as u8;
                fb[offset + 2] = check_color as u8;
                fb[offset + 3] = (check_color >> 24) as u8;
